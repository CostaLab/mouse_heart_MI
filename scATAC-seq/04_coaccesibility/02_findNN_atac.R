...
fname = file.path("save/", TimePoint,"a_atac_aggre_celltype.csv")
...
mtx_a_atac <- GetAssayData(a_atac, slot="counts", assay="peaks")
all_mtx_list <- lapply(all_celltypes, function(ct) mtx_a_atac[, names(all_list[[ct]])])
...
saveRDS(aggre_a_atac_mtx, file=glue("save/{TimePoint}/aggred_a_atac.Rds"))
