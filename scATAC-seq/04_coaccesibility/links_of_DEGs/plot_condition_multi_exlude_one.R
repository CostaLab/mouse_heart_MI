...
gene_to_group_factor <- list("S100a8"= c("CEBPB", "CEBPD"),
                             "S100a9" = c("CEBPB", "CEBPD"),
                             "Il1b" = c("IRF1", "IRF4", "IRF5")) 
...
for(up_gene in up_genes){
    ...
    jumpout <- FALSE 
    link_tracks  <- NULL
    df_links_Which_days <- switch(Exclude_Day, "Healthy"= list(df_links_Day3, df_links_Day10),
                                               "Day3" = list(df_links_Healthy, df_links_Day10),
                                               "Day10" = list(df_links_Healthy, df_links_Day3))

    Which_Days <-          switch(Exclude_Day, "Healthy"= list("Day3", "Day10"),
                                               "Day3" =  list("Healthy", "Day10"),
                                               "Day10" = list("Healthy", "Day3"))

    df_links_Which_day1 <- df_links_Which_days[[1]] 
    df_links_Which_day2 <- df_links_Which_days[[2]] 
    Which_Day1 <- Which_Days[[1]]
    Which_Day2 <- Which_Days[[2]]
    ...
    tryCatch({
        ...
        link_tracks <- plot_connections(connection_df = df_links_day_all, 
                                    ...
                                    minbp = minbp, 
                                    maxbp = maxbp,
                                    comparison_track = df_links_Which_days[[2]],
    ...
    jumpout <- FALSE
    df_links_Which_day1$conn_color <- condition_colors[Which_Day1]
    tryCatch({
        link_tracks <- plot_connections(connection_df = df_links_Which_day1, 
                                    gene_model = gene_model,
                                    gene_model_shape = "smallArrow",
                                    collapseTranscripts = "longest",
                     chr = chr, 
                     minbp = minbp, 
                     maxbp = maxbp,
                     comparison_track = df_links_Which_day1,
                     alpha_by_coaccess = FALSE,
                     # viewpoint = viewpoint,
                     connection_ymax = ymax,
                     comparison_ymax = ymax,
                     ...
                     comparison_connection_width = 1,
                     connection_color = "conn_color",
                     comparison_connection_color = condition_colors[Which_Day1],
                     include_axis_track = FALSE,
                     return_as_list = TRUE)
    }, error = function(cond) {
    ...
    link_track_Which_Day1 <- link_tracks[[1]]
    peak_track_Which_Day1 <- link_tracks[[2]]



    jumpout <- FALSE
    df_links_Which_day2$conn_color <- condition_colors[Which_Day2]
    tryCatch({
        link_tracks <- plot_connections(connection_df = df_links_Which_day2, 
                                    gene_model = gene_model,
                                    gene_model_shape = "smallArrow",
                                    collapseTranscripts = "longest",
                     chr = chr, 
                     minbp = minbp, 
                     maxbp = maxbp,
                     comparison_track = df_links_Which_day2,
                     alpha_by_coaccess = FALSE,
                     # viewpoint = viewpoint,
                     connection_ymax = ymax,
                     comparison_ymax = ymax,
                     ...
                     comparison_connection_width = 1,
                     connection_color = "conn_color",
                     comparison_connection_color = condition_colors[Which_Day2],
                     include_axis_track = FALSE,
                     return_as_list = TRUE)
    }, error = function(cond) {
        message("Error:", "Which_Day ", celltype, "-", up_gene)
    ...
    link_track_Which_Day2 <- link_tracks[[1]]
    peak_track_Which_Day2 <- link_tracks[[2]]



    gene_axis_track <- GenomeAxisTrack(fontsize = 4)
    
    link_track_Which_Day1@dp@pars$fontsize <- 12
    link_track_Which_Day2@dp@pars$fontsize <- 12
    data_track_Healthy@dp@pars$fontsize <- 12
    data_track_Day3@dp@pars$fontsize <- 12
    data_track_Day10@dp@pars$fontsize <- 12
    
    trackList <- list(link_track_Which_Day1,
                      peak_track_Which_Day1,
                      link_track_Which_Day2,
                      peak_track_Which_Day2,
                      data_track_Healthy,
                      data_track_Day3,
                      data_track_Day10,
                      gene_model_track, 
    ...
    pdf(glue("{scelltype}/condition_{scelltype}_{up_gene}_{tfs}.pdf"), height = 8, width = 8)
    plotTracks(trackList, title.width = 0.5, showTitle = TRUE, 
               from = minbp, to = maxbp, chromosome = chr, 
               sizes = c(0.4, 0.1, 0.4, 0.1, 0.3, 0.3, 0.3, 0.3, rep(0.2, length(tf_info_list)), 0.05), 
