...
peaks_files <- sprintf("../data/%s/unionPeaks.bed", samples)
...
df <- data.frame(seqnames=seqnames(SharedPeaks),
                 ...
                 name=paste0("peaks_", 1:length(SharedPeaks)),
                 score=score(SharedPeaks))
...
foreach(sample = samples) %dopar% {
    ...
    dir_1round <- paste0("../../1Round_Peakcalling/data/", sample) 
