...
samples <- c("sham", "day3", "day10")

#blacklist_file = "/work/sz753404/Blacklists/mm10-blacklist.v2.bed.gz"
blacklist_file = "../../../Blacklists/mm10-blacklist.v2.bed.gz"
blacklist <- rtracklayer::import.bed(blacklist_file)

## all chromosome range, chr1, chr2, .... chrX, chrY. length = 66
## coz, there's some chromosome chrUN_GL chr1_GLxxx_random..
chromSizes <- GRanges(names(seqlengths(BSgenome.Mmusculus.UCSC.mm10)), 
                      IRanges(1, seqlengths(BSgenome.Mmusculus.UCSC.mm10)))

## all chromosome range(remove some), chr1, chr2, .... chrX, chrY, chrM. len=22
chromSizes <- GenomeInfoDb::keepStandardChromosomes(chromSizes,
                                                    pruning.mode = "coarse")
      

peaks_files <- sprintf("../data/%s/Peaks/%s_summits.bed", samples, samples)
...
SharedPeaks <- nonOverlappingGRanges(unlist(gr_list), 
                                    by = "scoreQuantile", 
                                    decreasing = TRUE)
SharedPeaks <- sort(sortSeqlevels(SharedPeaks))

SharedPeaks <- SharedPeaks[seqnames(SharedPeaks) %in% paste0("chr",c(1:19,"X"))]
SharedPeaks <- keepSeqlevels(SharedPeaks, paste0("chr",c(1:19,"X")))

df <- data.frame(seqnames=seqnames(SharedPeaks),
                 starts=start(SharedPeaks)-1,
                 ends=end(SharedPeaks))

write.table(df, file = paste0( "../data/SharedPeaks.bed"), 
            sep = "\t", row.names = FALSE,
...
foreach(sample = samples) %dopar%{
    ...
    counts <- countInsertions(SharedPeaks, fragments, by = "RG")
    rownames(counts) <- paste(seqnames(SharedPeaks),
                              start(SharedPeaks),
                              end(SharedPeaks),
                              sep="_")
    ...
    saveRDS(counts, file = file.path(dir.out, "SharedPeaks_matrix.Rds"))
