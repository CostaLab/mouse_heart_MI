...
gene_to_factor = list("Col1a1"= c("RUNX1", "RUNX2", "BACH1", "BACH2", "SMAD3", "FOSL1", "NFKB1" ),
                      "Col1a2"= c("RUNX1", "RUNX2", "BACH1", "BACH2", "SMAD3", "FOSL1", "NFKB1" ),

                   "Pdgfra" = "CTCF") #CTCF CEBPG
#celltype <- "Fibroblasts"
#tf <- "RUNX2" 
#tf <- "CEBPG"
...
AllOptions <- function(){
    parser <- OptionParser()
    parser <- add_option(parser, c("-c", "--celltype"), type="character", default="Fibroblasts",
...
dict <- list(
    ...
    "Macrophages" = c("Nat9", "Cd244", "Clec4a1", "Adam8", "Rassf4", "Cd48", "Clec4n", "Dmkn", "Fcer1g", "Fyb", "Ifitm6", "Nckap1l", "Necap1", "Ptger2", "Slc7a7", "Tmsb4x", "Wfdc17"), 
...
for(up_gene in up_genes){
    ...
    for(a_tf in tf){
            ...
            tryCatch({
                ...
                message("Error:", "All ",celltype, "-", up_gene)
                jumpout <<- TRUE
            })   
            ...
            df_links_Healthy$conn_color <- condition_colors["Healthy"]
            jumpout <- FALSE
            tryCatch({
                    link_tracks <- plot_connections(connection_df = df_links_Healthy, 
                                            gene_model = gene_model,
                                            gene_model_shape = "smallArrow",
                                            collapseTranscripts = "longest",
                             chr = chr, 
                             ...
                             connection_color = "conn_color",
                             connection_color_legend = F,
                             comparison_connection_color = condition_colors['Healthy'],
                             include_axis_track = FALSE,
                             return_as_list = TRUE)


            }, error = function(cond) {
                message("Error:", "Healthy ",celltype, "-", up_gene)
                jumpout <<- TRUE
            })   
            ...
            link_track_Healthy <- link_tracks[[1]]
            peak_track_Healthy <- link_tracks[[2]]
            ...
            tryCatch({
                ...
                message("Error:", "Day3 ", celltype, "-", up_gene)
                jumpout <<- TRUE
            ...
            tryCatch({
                ...
                message("Error:", "Day10 ", celltype, "-", up_gene)
                jumpout <<- TRUE
            })   
            if(jumpout){
                next
            ...
            link_track_Healthy@dp@pars$fontsize <- 12
            ...
            trackList <- list(link_track_Healthy,
                              peak_track_Healthy,
                              link_track_Day3,
                              peak_track_Day3,
                              link_track_Day10,
            ...
            pdf(glue("{scelltype}/condition_{scelltype}_{up_gene}_{a_tf}_noTFnames.pdf"), height = 8, width = 8)
            plotTracks(trackList, title.width = 0.5, showTitle = TRUE, 
                       from = minbp, to = maxbp, chromosome = chr, 
                       sizes = c(0.4, 0.1,0.4, 0.1, 0.4, 0.1, 0.3, 0.3, 0.3, 0.3, 0.2, 0.05), 
